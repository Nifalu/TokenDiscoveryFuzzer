...
#ifdef LIBPNG_SILENCE_ERRORS
void my_png_error_fn(png_structp png_ptr, png_const_charp error_msg) {
  // fprintf(stderr, "[suppressed libpng error]: %s\n", error_msg);
  longjmp(png_jmpbuf(png_ptr), 1);
}

void my_png_warning_fn(png_structp, png_const_charp) {
  // No-op
}
#endif
...
#ifdef LIBPNG_SILENCE_ERRORS
  png_set_error_fn(png_handler.png_ptr, nullptr, my_png_error_fn, my_png_warning_fn);
#endif
